...
void print_data_abort_info(uint64 iss, uint64)
{
    ...
    panic_puts("\tDFSC (Data Fault Status Code): %s\n", dfsc_to_string(f.DFSC));

    panic_puts("\t\tWnR (Write not Read): %s\n",
    ...
    panic_puts("\t\tCM (Cache maintenance): %s\n",
    ...
    panic_puts("\t\tEA (External abort type): %s\n",
    ...
    panic_puts("\t\tFnV (FAR not Valid): %s\n",
               f.FnV ? "FAR is not valid, and holds an UNKNOWN value." : "FAR is valid.");

    panic_puts("\t\tBits[12:11]: %b\n", f.b12_11);

    panic_puts("\t\tBit[14]: %b\n", f.b14);
    panic_puts("\t\tBit[15]: %b\n", f.b15);

    panic_puts("\t\tBits[20:16]: %b\n", f.b20_16);

    panic_puts("\t\tSSE (Syndrome Sign Extend): %s\n",
               f.SSE ? "Data item must be sign-extended." : "Sign-extension not required.");

    switch (f.SAS) {
        case 0b00:
            panic_puts("\t\tSAS (Syndrome Access Size): Byte\n");
            break;
        case 0b01:
            panic_puts("\t\tSAS (Syndrome Access Size): Halfword\n");
            break;
        case 0b10:
            panic_puts("\t\tSAS (Syndrome Access Size): Word\n");
            break;
        case 0b11:
            panic_puts("\t\tSAS (Syndrome Access Size): Doubleword\n");
            break;
    }

    panic_puts("\t\tISV (Instruction Syndrome Valid): %s\n",
