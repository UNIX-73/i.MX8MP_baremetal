...
{
    return (bool)((dc.v >> MMU_DC_VALID_SHIFT) & MMU_DC_BITS(MMU_DC_VALID_WIDTH));
}

static inline mmu_descriptor_type dc_get_type(const mmu_hw_dc dc, mmu_granularity g,
                                              mmu_tbl_level l)
{
    DEBUG_ASSERT(l <= max_level(g));

    uint64 type_bit = (dc.v >> MMU_DC_TYPE_SHIFT) & MMU_DC_BITS(MMU_DC_TYPE_WIDTH);

    if (l == max_level(g)) {
        ASSERT(type_bit == 1, "descriptor_get_type: only page descriptors are valid");
...
{
    return (uint8)((dc.v >> MMU_DC_ATTR_INDEX_SHIFT) & MMU_DC_BITS(MMU_DC_ATTR_INDEX_WIDTH));
}
...
static inline mmu_access_permission dc_get_access_permissions(const mmu_hw_dc dc)
{
    return (mmu_access_permission)((dc.v >> MMU_DC_AP_SHIFT) & MMU_DC_BITS(MMU_DC_AP_WIDTH));
}

static inline mmu_shareability dc_get_shareability(const mmu_hw_dc dc)
{
    return (mmu_shareability)((dc.v >> MMU_DC_SH_SHIFT) & MMU_DC_BITS(MMU_DC_SH_WIDTH));
}
...
static inline p_uintptr dc_get_output_address(const mmu_hw_dc dc, mmu_granularity g)
{
...
{
    dc->v &= ~MMU_DC_FIELD_MASK(MMU_DC_ATTR_INDEX_SHIFT, MMU_DC_ATTR_INDEX_WIDTH);
    dc->v |= ((uint64)attr_index & MMU_DC_BITS(MMU_DC_ATTR_INDEX_WIDTH)) << MMU_DC_ATTR_INDEX_SHIFT;
}
...
static inline void dc_set_access_permissions(mmu_hw_dc* dc, mmu_access_permission permissions)
{
    dc->v &= ~MMU_DC_FIELD_MASK(MMU_DC_AP_SHIFT, MMU_DC_AP_WIDTH);
    dc->v |= ((uint64)permissions << MMU_DC_AP_SHIFT);
}

static inline void dc_set_shareability(mmu_hw_dc* dc, mmu_shareability shareability)
{
    dc->v &= ~MMU_DC_FIELD_MASK(MMU_DC_SH_SHIFT, MMU_DC_SH_WIDTH);
    dc->v |= ((uint64)shareability & MMU_DC_BITS(MMU_DC_SH_WIDTH)) << MMU_DC_SH_SHIFT;
}
...
static inline void dc_set_output_address(mmu_hw_dc* dc, p_uintptr output_address, mmu_granularity g)
{
    ...
    DEBUG_ASSERT(output_address % g == 0,
                 "dc_set_output_address: invalid output address, not aligned to granularity");
...
static inline void dc_set_software_defined(mmu_hw_dc* dc, uint8 software_defined)
{
    dc->v &= ~MMU_DC_FIELD_MASK(MMU_DC_SW_SHIFT, MMU_DC_SW_WIDTH);
    dc->v |= ((uint64)software_defined & MMU_DC_BITS(MMU_DC_SW_WIDTH)) << MMU_DC_SW_SHIFT;
}


static inline mmu_hw_dc td_build(mmu_tbl next, mmu_granularity g, isize_t physmap_offset)
{
    mmu_hw_dc dc = (mmu_hw_dc) {0};

    p_uintptr tbl_pa = (v_uintptr)next.dcs - physmap_offset;

    dc_set_type(&dc, MMU_DESCRIPTOR_TABLE);
    dc_set_output_address(&dc, tbl_pa, g);
    dc_set_valid(&dc, true);

    return dc;
}

static inline mmu_hw_dc bd_build(mmu_pg_cfg cfg, p_uintptr output_address, mmu_granularity g,
                                 mmu_tbl_level l)
{
    DEBUG_ASSERT(l <= max_level(g));
    ...
    dc_set_valid(&dc, true);
    dc_set_type(&dc, l == max_level(g) ? MMU_DESCRIPTOR_PAGE : MMU_DESCRIPTOR_BLOCK);
